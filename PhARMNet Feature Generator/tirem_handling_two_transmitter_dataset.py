...
def main():
    ...
    for ind, param_set in enumerate(param_list):
        ...
        # print("Didn't break",abcd)
        new_tx_data = tx_data[:, :, 1:].reshape(tx_data.shape[0] * 2, 2).clone().cpu().numpy()
        new_rx_data = np.zeros((tx_data.shape[0] * 2, rx_data.shape[1], rx_data.shape[2]))
        ...
        # for i in range(tx_data.shape[0] * 2):
        #     unique_sensors = list(sensor_info_dict.keys())
        #     for j, sensor_id in enumerate(unique_sensors):
        #         x_coord, y_coord, sensor_category = sensor_info_dict[sensor_id]
        #         new_rx_data[i, j, 1:] = [x_coord, y_coord, sensor_id, sensor_category]
        new_rx_dictionary = {}
        for j, name in enumerate(train_data):
            few_coords, rss, sensor_loc, keys, sensor_type = dataset[name]
            for sensor_id in keys:
                new_rx_dictionary[sensor_id] = [name, sensor_type]

        # print(new_rx_dictionary)
        # print(abcd)
        ...
        for i in range(tx_data.shape[0] * 2):
            ...
            for j, name in enumerate(train_data):
                few_coords, rss, sensor_loc, keys, sensor_type = dataset[name]
                for sensor_id in keys:
                    new_rx_data[i, row_index, 1:] = [sensor_loc[0], sensor_loc[1], sensor_id, sensor_type]
                    row_index += 1  # Move to the next row for the next key




        # print("New Tx data shape = ",new_tx_data.shape)
        # # # print("Rx data = ",rx_data[:5])

        # print("New Rx data = ",new_rx_data.shape)
        # print("New Rx data = ",new_rx_data[:5])
        # print(abcd)
        # print(abcd)
        ...
        for sensor_key in new_rx_dictionary.keys():
            ...
            input_features, tirem_tensor, tirem_db = get_tirem_features(rldataset=rldataset2, name=new_rx_dictionary[sensor_key][0], few_coords=new_tx_data, keys=keys)
            print(sensor_key)
            print(new_rx_dictionary[sensor_key][0])
            print(input_features.shape)
            # print(input_features[:5])
            model = TiremMLP([input_features.shape[-1],200,100])
            model_file = f'/uufs/chpc.utah.edu/common/home/u1472438/dl-image-localization-main/models_tirem_nn/AdvTr:False__arch:unet__Aug:None__CatMult:True__split:random__DS:6__elev:True__img_scale:25__randstate:0__test_size:0.2_{int(new_rx_dictionary[sensor_key][1])}.pt'
            ...
            with torch.no_grad():
                predicted_rss = model(input_features, tirem_tensor)
            
            # print(new_tx_data.shape)
            # print(predicted_rss.shape)
            # print(predicted_rss[:5])
            
            min_rss, max_rss = rldataset.get_min_max_rss_from_key(sensor_key)
            final_predicted_rss = predicted_rss * (max_rss - min_rss) + min_rss       
            # print(final_predicted_rss[:5])
            print(final_predicted_rss.shape)
            final_predicted_rss_np = final_predicted_rss.cpu().numpy().flatten() 
            matching_indices = np.where(new_rx_data[:, :, 3] == sensor_key)
            new_rx_data[matching_indices[0], matching_indices[1], 0] = final_predicted_rss_np[matching_indices[0]]
        
        combined_tx_data = np.stack([new_tx_data[::2], new_tx_data[1::2]], axis=1) 
        combined_rx_data = new_rx_data[::2].copy() 
        rss_0 = new_rx_data[::2, :, 0]
        rss_1 = new_rx_data[1::2, :, 0]

        power_0 = 10 ** (rss_0 / 10)
        power_1 = 10 ** (rss_1 / 10)

        power_total = power_0 + power_1

        rss_total = 10 * np.log10(power_total)

        combined_rx_data[:, :, 0] = rss_total 
        zero_indices = combined_rx_data[:, :, 4] == 0 
        combined_rx_data[zero_indices, 0] = 0
        
        tx_data = tx_data.cpu().numpy()
        rx_data = rx_data.cpu().numpy()
        
        for i in range(0,rx_data.shape[0]):
            for j in range(0,40):
                if rx_data[i,j,3] != 0:
                    sensor_id = int(rx_data[i,j,3])
                    min_rss, max_rss = rldataset.get_min_max_rss_from_key(sensor_id)
                    rx_data[i,j,0]= rx_data[i,j,0]* (max_rss - min_rss) + min_rss  
        ...
        csv_file_path = "/uufs/chpc.utah.edu/common/home/u1472438/dl-image-localization-main/final_report_two_tx_tirem.csv"
        with open(csv_file_path, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(csv_header)
            for i in range(combined_tx_data.shape[0]):
                sample_label = f"{i+1}"
                row = [sample_label]
                row.extend(tx_data[i].flatten())

                sensor_values = {name: 0 for name in sensor_names}
                for j in range(rx_data.shape[1]):
                    sensor_id = int(rx_data[i, j, 3])
                    if sensor_id in location_index.values() and sensor_id != 0:
                        ...
                        sensor_values[sensor_name] = rx_data[i, j, 0]
                row.extend(sensor_values.values())
                writer.writerow(row)

                sample_label = f"tirem-{i+1}"
                row = [sample_label]
                row.extend(combined_tx_data[i].flatten())

                sensor_values = {name: 0 for name in sensor_names}
                for j in range(combined_rx_data.shape[1]):
                    sensor_id = int(combined_rx_data[i, j, 3])
                    if sensor_id in location_index.values() and sensor_id != 0:
                        sensor_name = [name for name, idx in location_index.items() if idx == sensor_id][0]
                        sensor_values[sensor_name] = combined_rx_data[i, j, 0]
                row.extend(sensor_values.values())
                writer.writerow(row)               

                sample_label = f"tirem_solo-{1}"
                row = [sample_label]
                row.extend(new_tx_data[i*2].flatten())
                empty_array=[0,0]
                row.extend(empty_array)

                sensor_values = {name: 0 for name in sensor_names}
                for j in range(new_rx_data.shape[1]):
                    sensor_id = int(new_rx_data[i*2, j, 3])
                    if sensor_id in location_index.values() and sensor_id != 0:
                        sensor_name = [name for name, idx in location_index.items() if idx == sensor_id][0]
                        sensor_values[sensor_name] = new_rx_data[i*2, j, 0]
                row.extend(sensor_values.values())
                writer.writerow(row)     

                sample_label = f"tirem_solo-{2}"
                row = [sample_label]
                empty_array=[0,0]
                row.extend(empty_array)
                row.extend(new_tx_data[(i*2)+1].flatten())

                sensor_values = {name: 0 for name in sensor_names}
                for j in range(new_rx_data.shape[1]):
                    sensor_id = int(new_rx_data[(i*2)+1, j, 3])
                    if sensor_id in location_index.values() and sensor_id != 0:
                        sensor_name = [name for name, idx in location_index.items() if idx == sensor_id][0]
                        sensor_values[sensor_name] = new_rx_data[(i*2)+1, j, 0]
                row.extend(sensor_values.values())
                writer.writerow(row)       
        ...
        for k in range(0,len(all_device_names)):
            ...
            # print("Name = ", name)
            if not os.path.exists(f'/uufs/chpc.utah.edu/common/home/u1472438/dl-image-localization-main/tirem_features2/tirem_all_features_DS{rldataset.params.dataset_index}_{name}.npz'):
